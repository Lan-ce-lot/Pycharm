...
class idiom():
    ...
    def idiom_test(self,idiom1, idiom2, mode, opt):
        """判断两个成语是否达成接龙条件"""
        #为了可读性，我把它分开写，比较清晰
        if mode == 0 and idiom2[0] != idiom1[-1]:
            return False
        if mode == 1 and self.chinese_to_pinyin(idiom2[0]) != self.chinese_to_pinyin(idiom1[-1]):
            return False
        if mode ==2 and self.chinese_to_pinyin(idiom2[0])[:-2] != self.chinese_to_pinyin(idiom1[-1])[:-2]:
            return False
        if opt == 0 and len(idiom2) != 4:
            return False
        return True

    def idiom_select(self, x, mode, opt):
        ...
        if x == None:
            ...
            with open('idiom.txt','r') as f:
                ...
                pinyin = self.chinese_to_pinyin(x[-1])
                base = f.readlines()
                if pinyin[0] != 'Z':
                ...
                for i in base:
                    ...
                    if mode == 0 and i[0] == x[-1]:
                        return i[:-1]
                    if mode == 1 and self.chinese_to_pinyin(i[0]) == pinyin:
                        return i[:-1]
                    if mode == 2 and self.chinese_to_pinyin(i[0])[:-2] == pinyin[:-2]:
                        return i[:-1]
            return None

    def idiom_start(self, start = 0, mode = 0, opt = 0):
        """start参数表示先后手，0表示电脑先手，1表示玩家先手；返回值代表游戏结果，为0表示玩家失败，为1代表玩家胜利"""
        memory = set()  #记忆集合，用于判断成语是否被重复使用
        if start == 0:
            while True:
                t = self.idiom_select(None, mode, opt)
                if self.idiom_select(t, mode, opt) != None:
                    break
            print(t)
        else:
            p = input("请输入成语:")
            ...
            while True:
                t = self.idiom_select(p, mode, opt)
                cycle_flag += 1
                if t not in memory:
                    break
                if cycle_flag == 10:
        ...
        while True:
            ...
            if p in memory:
                print("游戏结束！该成语已被使用过")
                return 0
            if self.idiom_test(t, p, mode, opt) == False:
                print("游戏结束！你未遵守游戏规则")
                return 0
            memory.add(p)
            cycle_flag = 0
...
if __name__ == '__main__':
    ...
    I = idiom()
    I.idiom_start(start=1, mode=2, opt=1)